/* ═══════════════════════════════════════════
   Appearance Store — Renk semasi, ton, accent, blur, chrome bg yonetimi
   Yan etki: localStorage okuma/yazma, CSS custom property set
   Build output: js/appearance-store.js
═══════════════════════════════════════════ */

import type {
  AppearanceState, AppearanceStoreAPI,
  TonePreset, AccentPreset, RGB, HSL, EventCallback,
} from '../../shared/types';

// ── Preset Data ──────────────────────────

const LIGHT_TONES: TonePreset[] = [
...
function emit(event: string, payload: unknown): void {
  (listeners[event] || []).forEach(cb => { try { cb(payload); } catch (e) { console.error('AppearanceStore event error:', e); } });
}
...
function load(): void {
  try {
    ...
    if (legacyAccent) {
      // Migrate once: write split keys, remove legacy key
...
function applyToneVars(t: TonePreset): void {
...
function applyAccentVars(key: string): void {
...
function setMode(m: 'dark' | 'light'): void {
  state.mode = m; save(); applyMode(m);
  // Reapply accent for the new mode
  applyAccentVars(m === 'dark' ? state.accentDark : state.accentLight);
  emit('mode-change', { mode: m });
  emit('any-change', { state: getState() });
}

function setLightTone(key: string): void {
  state.lightTone = key; save();
  if (state.mode === 'light') applyMode('light');
  emit('tone-change', { mode: 'light', tone: key });
  emit('any-change', { state: getState() });
}

function setDarkTone(key: string): void {
  state.darkTone = key; save();
...
function setAccent(key: string): void {
  state.accentDark = key; state.accentLight = key; save(); applyAccentVars(key);
  emit('accent-change', { key, color: (ACCENTS.find(x => x.key === key) ?? ACCENTS[6])?.color ?? '' });
  emit('any-change', { state: getState() });
}

function setAccentDark(key: string): void {
  state.accentDark = key; save();
  if (state.mode === 'dark') applyAccentVars(key);
  emit('accent-change', { mode: 'dark', key, color: (ACCENTS.find(x => x.key === key) ?? ACCENTS[6])?.color ?? '' });
  emit('any-change', { state: getState() });
}

function setAccentLight(key: string): void {
  state.accentLight = key; save();
  if (state.mode === 'light') applyAccentVars(key);
  emit('accent-change', { mode: 'light', key, color: (ACCENTS.find(x => x.key === key) ?? ACCENTS[6])?.color ?? '' });
  emit('any-change', { state: getState() });
}

function setCustomLight(hex: string): void {
  state.customLight = hex; state.lightTone = 'custom'; save();
  if (state.mode === 'light') applyMode('light');
  emit('tone-change', { mode: 'light', tone: 'custom' });
  emit('any-change', { state: getState() });
}

function setCustomDark(hex: string): void {
  state.customDark = hex; state.darkTone = 'custom'; save();
  if (state.mode === 'dark') applyMode('dark');
  emit('tone-change', { mode: 'dark', tone: 'custom' });
  emit('any-change', { state: getState() });
}

function setBlur(level: number): void {
...
function setChromeBg(hex: string | null): void {
...
const store: AppearanceStoreAPI = {
  getState, apply, on, off,
  setMode, setLightTone, setDarkTone,
  setAccent, setAccentDark, setAccentLight,
  setCustomLight, setCustomDark, setBlur, setChromeBg,
  getChromeBgResolved,
